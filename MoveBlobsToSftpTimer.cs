...
namespace AUFS
{
    public class MoveBlobsToSftpTimer
    {
        private readonly ILogger<MoveBlobsToSftpTimer> _logger;
        ...
        public MoveBlobsToSftpTimer(ILogger<MoveBlobsToSftpTimer> logger)
        {
            ...
            sftpBasePath = Environment.GetEnvironmentVariable("SFTP_BASE_PATH") ?? "/Test/Inbound/";
        }

        [Function("MoveBlobsToSftpTimer")]
        public async Task RunAsync([TimerTrigger("0 * * * * *")] TimerInfo myTimer)
        {
            ...
            var containerClient = blobServiceClient.GetBlobContainerClient("exportcontainer");
